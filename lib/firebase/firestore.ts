...
/**
 * Get user's UI preferences (zoomLevel, darkMode)
 */
export async function getUserUIPreferences(userId: string): Promise<{ zoomLevel?: number; darkMode?: boolean }> {
  try {
    const userRef = doc(getUsersCollection(), userId);
    const userSnap = await getDoc(userRef);
...
/**
 * Set user's UI preferences (zoomLevel, darkMode)
 */
export async function setUserUIPreferences(userId: string, preferences: { zoomLevel?: number; darkMode?: boolean }) {
  try {
    const userRef = doc(getUsersCollection(), userId);
