...
export async function getUserDefaultBoard(userId: string): Promise<string | null> {
  try {
    ...
    console.error('Error getting user default board:', error);
    return null;
  }
}
...
export async function setUserDefaultBoard(userId: string, boardId: string | null) {
  try {
    ...
    console.log('Set default board for user:', userId, 'boardId:', boardId);
  } catch (error) {
    console.error('Error setting user default board:', error);
    throw error;
  }
}
