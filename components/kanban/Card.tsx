...
const Card = ({ card, boardId, columnId, onDragStart, onDragEnd, isDragging, canEdit }: CardProps) => {
  ...
  // Helper: get text color for a colored label
  const getLabelTextColor = (bg: string) => isLightColor(bg) ? '#374151' : '#ffffff';
      ...
      >
        ...
        <div className="p-4 flex flex-col gap-2 flex-1">
          ...
          <div className="flex flex-wrap gap-1.5 pointer-events-none">
            ...
            {/* Tags — colored labels when tagColors set */}
            {card.tags && card.tags.slice(0, 2).map((tag) => {
              const bg = card.tagColors?.[tag];
              return (
                <span
                  key={tag}
                  className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
